...
table(coord_data$Habitat)

# Read ASV data
asv_data <- read.csv("16SASV_final.csv", row.names = 1, check.names = FALSE)
print(paste("\nASV data dimensions:", nrow(asv_data), "ASVs x", ncol(asv_data), "samples"))
...
print(paste("\nSamples in coordinates:", length(coord_samples)))
...
print(paste("ASVs retained:", ncol(asv_t)))
...
if(length(pond_samples) >= 4) {
  ...
  pond_mantel <- perform_mantel(geo_dist_pond, comm_dist_pond, "Ponds only", 
                                pond_samples, "Pond")
...
if(length(mat_samples) >= 4) {
  ...
  mat_mantel <- perform_mantel(geo_dist_mat, comm_dist_mat, "Desiccated only", 
...
# 2. Distance-decay plots (Combined and Separate)
# Prepare data for combined plot
geo_vec <- as.vector(geo_dist_obj)
comm_vec <- as.vector(comm_dist)

# Create habitat pairs - simpler approach
...
cat("Plot data dimensions:", nrow(plot_data_combined), "rows\n")
cat("Habitat pair types:", levels(plot_data_combined$Habitat_Pair), "\n")

# Combined distance-decay plot
p2 <- ggplot(plot_data_combined, aes(x = Geographic_Distance_km, y = Community_Dissimilarity, 
                                     color = Habitat_Pair)) +
  geom_point(alpha = 0.6, size = 2) +
  geom_smooth(method = "lm", se = TRUE, size = 1.2) +
  scale_color_manual(values = c("Desiccated-Desiccated" = "#A23B72", "Desiccated-Pond" = "#F18F01", "Pond-Pond" = "#2E86AB"),
                     na.value = "grey50") +
  theme_minimal() +
  labs(title = "Distance-Decay Relationship: Combined Analysis",
       subtitle = paste("Mantel r =", round(combined_mantel$result$Mantel_r, 3), 
...
# 3. Separate habitat plots (if both have enough samples)
...
if(exists("pond_mantel") && exists("mat_mantel")) {
  ...
  if(length(pond_samples) >= 4) {
    ...
    p3 <- ggplot(plot_data_pond, aes(x = Geographic_Distance_km, y = Community_Dissimilarity)) +
      ...
      theme_minimal() +
      labs(title = "Distance-Decay: Ponds Only",
  ...
  if(length(mat_samples) >= 4) {
    ...
    p4 <- ggplot(plot_data_mat, aes(x = Geographic_Distance_km, y = Community_Dissimilarity)) +
      ...
      theme_minimal() +
      labs(title = "Distance-Decay: Mats Only",
           subtitle = paste("Mantel r =", round(mat_mantel$result$Mantel_r, 3), 
...
# Check NMDS results and extract scores properly
if(nmds$converged) {
  ...
  # Ensure nmds_scores is a matrix with 2 columns
  if(is.null(dim(nmds_scores)) || ncol(nmds_scores) != 2) {
    cat("Warning: NMDS scores structure unexpected. Trying alternative extraction...\n")
  ...
  p5 <- ggplot(nmds_df, aes(x = NMDS1, y = NMDS2, color = Habitat)) +
    ...
    scale_color_manual(values = habitat_colors) +
    stat_ellipse(level = 0.68, linetype = 2, size = 1) +
    theme_minimal() +
  ...
  cat("This can happen with high-dimensional data or insufficient samples.\n")
}

# 5. Hierarchical clustering with habitat colors
hc <- hclust(comm_dist, method = "average")
plot(hc, main = "Hierarchical Clustering of Microbial Communities", 
...
ggsave("01_sample_locations_final.png", plot_list[[1]], width = 10, height = 8, dpi = 300)
ggsave("02_distance_decay_combined_final.png", plot_list[[2]], width = 10, height = 8, dpi = 300)

if(length(plot_list) >= 3) {
  ggsave("03_distance_decay_ponds_final.png", plot_list[[3]], width = 10, height = 8, dpi = 300)
}

if(length(plot_list) >= 4) {
  ggsave("04_distance_decay_mats_final.png", plot_list[[4]], width = 10, height = 8, dpi = 300)
}

if(length(plot_list) >= 5) {
  ggsave("05_NMDS_ordination_final.png", plot_list[[5]], width = 10, height = 8, dpi = 300)
}

# Save combined plot panel
if(length(plot_list) >= 4) {
  combined_plot <- grid.arrange(plot_list[[1]], plot_list[[2]], plot_list[[3]], plot_list[[4]], 
                                ncol = 2, nrow = 2)
  ggsave("06_combined_analysis_panel_final.png", combined_plot, width = 16, height = 12, dpi = 300)
...
png("07_hierarchical_clustering_final.png", width = 12, height = 8, units = "in", res = 300)
plot(hc, main = "Hierarchical Clustering of Microbial Communities", 
     sub = paste("Bray-Curtis dissimilarity |", ncol(asv_t), "ASVs |", nrow(asv_t), "samples"),
     xlab = "", cex = 0.7)
habitat_colors_vec <- habitat_colors[coord_filtered$Habitat[match(hc$labels, coord_filtered$SampleID)]]
...
cat("Plots saved:\n")
cat("- 01_sample_locations_final.png\n")
cat("- 02_distance_decay_combined_final.png\n")
if(length(plot_list) >= 3) cat("- 03_distance_decay_ponds_final.png\n")
if(length(plot_list) >= 4) cat("- 04_distance_decay_mats_final.png\n")
if(length(plot_list) >= 5) cat("- 05_NMDS_ordination_final.png\n")
cat("- 06_combined_analysis_panel_final.png\n")
cat("- 07_hierarchical_clustering_final.png\n")
...
cat("\n- Total ASVs retained:", ncol(asv_t))
cat("\n- Habitat distribution:")
...
# analysis_results$coordinate_data
# etc.
